...
export default function ViewJobs() {
      ...
      try {
        ...
        const jobsData = await Promise.all(
          jobIds.map(async (jobId) => {
            ...
            console.log(`Fetched IPFS Data for Job ID ${jobId}:`, ipfsData);

            return {
              jobId,
              title: ipfsData.title || null, // Assuming title is fetched from IPFS
              employer: ipfsData.jobGiver || null, // Fetch job giver from IPFS data
              jobTaker: ipfsData.jobTaker || null, // Fetch job taker from IPFS data
  ...
  const fetchFromIPFS = async (hash) => {
    try {
      ...
      return { title: null, jobGiver: null, jobTaker: null }; // Return null if title isn't found
    }
  };
          ...
          <table className="table">
            <thead>
              <tr>
                <th>Job Title</th>
                <th>From</th>
                <th></th> {/* Column for the arrow icon */}
                <th>To</th>
                <th>Amount Paid</th>
                <th>Status</th>
                <th></th>
              ...
              {currentJobs.map((job) => (
                <tr key={job.jobId}>
                  ...
                  <td className="from-to-icon-cell">
                    <img src="/rightarrow.svg" alt="Arrow Icon" className="from-to-icon" />
                  </td>
                  <td>{truncateAddress(job.jobTaker)}</td> {/* New "To" column data */}
